  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (modification === "edit") {
        ...
        if (!delResponse.ok) {
          console.error(`Error deleting the course days (course days reset)`);
          return null;
        }
        const dayIDs: number[] = [];
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (!response.ok) {
            console.error(`Failed fetching dayID`);
            return null;
          }
        ...
        for (let i = 0; i < dayIDs.length; i++) {
          ...
          if (!response.ok) {
            console.error(`Failed INSERTING dayIDs`);
            return null;
          }
          const data = await response.json();
          console.log(data);
  ...
  return (
    <>
      <div className="relative justify-center items-center flex-col mx-auto my-20 w-1/2 max-w-screen-md">
        ...
        {conflictMessage && (
          <div className="text-red-600 mt-4">{conflictMessage}</div>
        )}
      </div>
    </>
  );
